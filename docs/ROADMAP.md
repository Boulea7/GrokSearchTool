...
- Keep advanced `deep research` diagnostics visible in detailed tooling without letting them redefine the default lightweight health story for `plan_* -> web_search`
- Improve source provenance, confidence cues, claim traceability, and result ordering so useful evidence is surfaced before low-value noise
