...
- `connection_test` сейчас отражает только достижимость `/models`; если `web_search` находится в состоянии `degraded`, нужно смотреть на `doctor`, `feature_readiness`, `GROK_MODEL_SOURCE` и проверки `grok_model_selection` / `grok_search_probe` вместе.
