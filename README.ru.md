...
| `GROK_MCP_DISABLED_TOOL_GROUPS` | Нет | Необязательный список групп MCP-инструментов, которые нужно скрыть: `planning`, `deep_research`, `host_controls`. Неизвестные непустые значения приводят к ошибке запуска, чтобы опечатки не игнорировались. Пустое значение публикует все `21` инструмента; core search / fetch tools этим параметром не отключаются |
