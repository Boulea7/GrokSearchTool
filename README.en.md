...
`sources.json` now carries additive source-quality metadata such as `source_key`, `quality_score`, and `quality_tier` alongside `source_id`. Claims inside `citations.json` and `report.json` may also include additive `unit_id` and `evidence_ids` provenance fields. Continuation rebuilds now prefer `sources.json`, but can fall back to `citations.json.source_registry` when the sources artifact is missing or unreadable. Jobs recovered from in-flight `queued` or `running` state are now reconciled into `interrupted` with an explicit recovery reason. `resume` now starts a new attempt time window instead of replaying the previous terminal timestamps.
