...
GrokSearch は、安定した Web リサーチを必要とするアシスタントや汎用クライアント向けに独立運用されている MCP サーバーです。

`Grok` の検索能力と `Tavily` / `Firecrawl` の抽出能力を組み合わせ、単純な検索から多段階リサーチまで扱える MCP ツール群を提供します。
