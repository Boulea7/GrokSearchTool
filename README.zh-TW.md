...
GrokSearch 是一個獨立維護的 MCP 伺服器，面向需要穩定網頁研究流程的助理與通用客戶端。

它整合 `Grok` 搜尋與 `Tavily`、`Firecrawl` 擷取能力，提供同時適用於單跳查詢與多階段研究任務的 MCP 工具面。
...
- `web_map`：網站結構映射
- `plan_*`：複雜研究任務的分階段規劃工具
- `get_config_info`：檢查設定並測試 `/models`
